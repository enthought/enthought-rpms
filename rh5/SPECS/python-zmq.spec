...
%build
CFLAGS="%{optflags}" %{__python} setupegg.py fetch_libzmq build --zmq=bundled

%if 0%{?with_python3}
pushd %{py3dir}
CFLAGS="%{optflags}" %{__python3} setup.py fetch_libzmq build --zmq=bundled
popd
%endif # with_python3
...
%if 0%{?with_python3}
pushd %{py3dir}
%{__python3} setup.py install --zmq=bundled --skip-build --root %{buildroot}
...
%{__python} setupegg.py install --zmq=bundled -O1 --skip-build --root %{buildroot}
